...
def main():
    ...
    # This is a good configuration of CRNL for hyperspectral data
    data = "om18"
    c = "3"
    max_iter = 15001
    omega_0_4D = 6
    ...
    for iter in range(max_iter):
        ...
        loss = loss + gamma * torch.norm(
            X_Out_real[1:,:,:]-X_Out_real[:-1,:,:], 1)
        loss = loss + gamma * torch.norm(
            X_Out_real[:,1:,:]-X_Out_real[:,:-1,:], 1)
        
        dz = X_Out_real[:,:,1:]-X_Out_real[:,:,:-1]
        
        loss = loss + 10*gamma * torch.norm(
            dz[1:,:,:]-dz[:-1,:,:], 1)
        loss = loss + 10*gamma * torch.norm(
            dz[:,1:,:]-dz[:,:-1,:], 1)
